...
let tile = document.querySelector('.card-row')
let addtile= ``
for(object of data) {
    addtile = addtile+`<div class='card' style="background-color: ${object.color}">
                            <h3>${object.topic}</h3>
                            <h2>${object.title}</h2>
                            <button> Read for ${object.price}</button>
                            </div>`
}
tile.innerHTML=addtile
